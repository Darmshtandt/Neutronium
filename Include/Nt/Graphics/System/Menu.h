...
namespace Nt {
	...
	public:
		...
		using SubMenuContainer = std::vector<std::unique_ptr<Menu>>;
	...
	private:
		_NODISCARD Item* _GetItemPtrFromID(const uInt& id);
		void _Modify(const uInt& flags, const uInt& id, const std::wstring& text) const;
