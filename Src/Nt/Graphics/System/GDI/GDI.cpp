...
namespace Nt::GDI {
	HBITMAP CreateCompatibleBitmap(const HDC& hdc, const uInt2D& size) {
		if (size.x == 0 && size.y == 0)
			Log::Instance().Warning("Size = 0");
		else if (size.x == 0)
			Log::Instance().Warning("Width = 0");
		else if (size.y == 0)
	...
	void GetDIBits(const HDC& hdc, const HBITMAP& hBitmap, const uInt& start, const uInt& height, void* pBits, BITMAPINFO* pBmpInfo, const uInt& usage) {
		RequireNotNull(hdc);
	...
	Bool StretchBlt(const HDC& hdcDest, const IntRect& rectDest, const HDC& hdcSrc, const IntRect& rectSrc, const DWord& rop) {
		RequireNotNull(hdcDest);
		RequireNotNull(hdcSrc);

		if (rectDest.Right == 0 && rectDest.Bottom == 0)
			Log::Instance().Warning("Destination size = 0");
		else if (rectDest.Right == 0)
			Log::Instance().Warning("Destination width = 0");
		else if (rectDest.Bottom == 0)
			Log::Instance().Warning("Destination height = 0");

		if (rectSrc.Right == 0 && rectSrc.Bottom == 0)
			Log::Instance().Warning("Source size = 0");
		else if (rectSrc.Right == 0)
			Log::Instance().Warning("Source width = 0");
		else if (rectSrc.Bottom == 0)
			Log::Instance().Warning("Source height = 0");
	...
	HBITMAP ScaleHBitmap(const HDC& hdc, const HBITMAP& hBitmap, const IntRect& bitmapRect, const IntRect& scaleRect) {
