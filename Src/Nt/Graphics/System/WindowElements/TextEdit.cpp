...
namespace Nt {
	...
	TextEdit::TextEdit(const HWND& hwnd) :
		HandleWindow(hwnd) 
	{
		if (m_hwnd != nullptr) {
	...
	TextEdit::TextEdit(const String& text, const Bool& isRich) {
		Create(text, isRich);
	}

	TextEdit::TextEdit(const IntRect& windowRect, const String& text, const Bool& isRich) {
		Create(windowRect, text, isRich);
	}
	TextEdit::TextEdit(TextEdit&& edit) noexcept :
	...
	void TextEdit::Create(const String& text, const Bool& isRich) {
	...
	void TextEdit::Create(const IntRect& windowRect, const String& text, const Bool& isRich) {
		if (IsCreated()) {
	...
	void TextEdit::Create(const IntRect& windowRect, const String& text) {
		m_Name = text;
...
#if 0
	LRESULT CALLBACK TextEdit::_TextEditProc(HWND hwnd, uInt uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData) {
	...
	void TextEdit::OnUpdate() {
		if (m_OnUpdate)
			m_OnUpdate();
	}

	Nt::String TextEdit::GetText() const noexcept {
		if (!IsCreated())
			return "";
	...
	void TextEdit::SetOnUpdate(std::function<void()> onUpdate) {
		m_OnUpdate = onUpdate;
	}

	void TextEdit::SetBackgroundColor(const Byte3D& color) {
		m_BackgroundColor = color;
		if (m_hwnd != nullptr)
			SendMessage(m_hwnd, EM_SETBKGNDCOLOR, 0, VectorToColorRef(m_BackgroundColor));
	}

	void TextEdit::SetTextColor(const Byte3D& color) noexcept {
		m_TextColor = color;
	...
	void TextEdit::SetTextWeight(const uInt& weight) noexcept {
		m_TextWeight = weight;
	...
	void TextEdit::SetPlaceholder(const Nt::String& placeholder) {
		m_Placeholder = placeholder;
		if (m_hwnd != nullptr)
			SendMessage(m_hwnd, EM_SETCUEBANNER, TRUE, reinterpret_cast<LPARAM>(m_Placeholder.c_str()));
	}
