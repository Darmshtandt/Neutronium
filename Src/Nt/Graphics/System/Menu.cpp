...
namespace Nt {
	std::unordered_map<HMENU, Int> g_MenuHandleMap;
		...
		m_Handle(hMenu)
	{
		...
		for (uInt i = 0; i < itemCount; ++i) {
			...
			if (hSubmenu)
				m_SubMenu.emplace_back(new Menu(hSubmenu));
		}
	}
	...
	{
		for (const std::unique_ptr<Menu>& pMenu : menu.m_SubMenu)
			m_SubMenu.emplace_back(new Menu(*pMenu));
	...
	void Menu::AddSubMenu(const Menu& subMenu, const Flag& flags, const String& text) {
		m_SubMenu.emplace_back(new Menu(subMenu.m_Handle));
