...
namespace Nt {
	NT_CONSTEXPR Face::Face(const Vertex& a, const Vertex& b, const Vertex& c) noexcept :
		Vertices { a, b, c }
	{
	}
	
	NT_CONSTEXPR void Face::Split(const Plane& plane, std::vector<Face>& frontPart, std::vector<Face>& backPart) const {
	...
	NT_CONSTEXPR Float3D Face::GetNormal() const noexcept {
		const Float3D edge1 = (Vertices[1].Position - Vertices[0].Position).xyz;
		const Float3D edge2 = (Vertices[2].Position - Vertices[0].Position).xyz;
		return edge1.GetCross(edge2);
	}
	NT_CONSTEXPR Float3D Face::GetCenter() const noexcept {
		Float3D center;
	...
	NT_CONSTEXPR Edge Face::GetEdge(const uInt& index) const {
		Assert(index < 3, "Out of range");

		return Edge(Vertices[index], Vertices[(index + 1) % 3]);
	}

	NT_CONSTEXPR Bool Face::HasIntersection(const Face& face) const noexcept {
		Float3D temp;
		return HasIntersection(face, temp);
	}
	NT_CONSTEXPR Bool Face::HasIntersection(const Face& face, Float3D& intersectionPoint) const noexcept {
	...
	NT_CONSTEXPR Bool Face::HasPointInEdge(const Float3D& point) const noexcept {
		for (uInt i = 0; i < 3; ++i) {
	...
	NT_CONSTEXPR Bool Face::IsConnected(const Face& other) const {
	...
	NT_CONSTEXPR Bool Face::operator==(const Face& face) const noexcept {
		for (uInt i = 0; i < 3; ++i) {
	...
	NT_CONSTEXPR void Face::SetEdge(const Edge& edge, const uInt& index) {
		Assert(index < 3, "Out of range");
